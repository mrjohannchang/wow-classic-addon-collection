...
-- Widget
---------------------------------------------------------------------------------------------------------
local widgetPath = "Interface\\Addons\\NeatPlatesWidgets\\HealerWidget"
...
NeatPlatesWidgets.CreateHealerWidget = CreateWidgetFrame
...
local function UpdateRolesViaScoreboard()
	...
	if NumScores > 0 then
		...
		if UpdateIsNeeded then NeatPlates:RequestDelegateUpdate() end
	end
		...
		if SpellEvents[combatevent] then								-- Filter: Specific spell events
			if HealerSpells[spellid] then								-- Filter: Known Healing Spells
				...
				if RoleList[rawName] ~= "Healer" then
					...
					NeatPlates:RequestDelegateUpdate()
				end
			end
		end
	end
...
NeatPlatesUtility.EnableHealerTrack = Enable
NeatPlatesUtility.DisableHealerTrack = Disable
NeatPlatesUtility.IsHealer = IsHealer
